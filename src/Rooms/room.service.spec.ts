...
describe('RoomService', () =>{
  ...
  describe('getRooms', () => {
    ...
    it("Should call prisma room.findMany with correct params", async () =>{
      ...
      jest.spyOn(prismaService.room, "findMany").mockImplementation(mockPrismaFindManyRoom)
    ...
    it("Should return throw not found exception if there's no rooms availabe", async() => {
      ...
      const mockPrismaFindManyBookings = jest.fn().mockReturnValue(mockBookings)
      jest.spyOn(prismaService.bookings, "findMany").mockImplementation(mockPrismaFindManyBookings)

      await expect(service.getRooms(filters)).rejects.toThrowError(NotFoundException)
    })
  })
  ...
  describe("getRoomById", () =>{
    it("Should call prisma room.findUnique with correct params", async () => {
      ...
      jest.spyOn(prismaService.room, "findUnique").mockImplementation(mockPrismaFindUniqueRoom)
    ...
    it("Should throw not found exception provided id is not valid", async () =>{
      ...
      await expect(service.getRoomById(1)).rejects.toThrowError(NotFoundException)
    })
  })
