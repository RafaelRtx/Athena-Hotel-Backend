...
export function SwaggerGetRoomsDecorator() {
  return applyDecorators(
    ApiOkResponse({ status: 200,description: 'Return all available Rooms in specified date', type:RoomResponseDto}),
    ApiNotFoundResponse({ status: 401, description: 'No rooms found in specified date', type:ErrorResponse}),
...
export function SwaggerGetRoomByIdDecorator() {
  return applyDecorators(
    ApiOkResponse({ status: 200,description: 'Return Room by id', type:RoomResponseDto}),
