...
describe("AuthService", () =>{
  ...
  describe("signup", () => {
    it("Should call prisma.create with correct params", async () =>{
      ...
      jest.spyOn(prismaService.guest, "create").mockImplementation(mockPrismaCreateGuest)
      const mockPrismaGetGuest = jest.fn().mockReturnValue(null)
      jest.spyOn(prismaService.guest, "findUnique").mockImplementation(mockPrismaGetGuest)
    ...
    it("Should throw conflict exception if user already exists", async ()=> {
      ...
      jest.spyOn(prismaService.guest, "findUnique").mockImplementation(mockExistingGuest)

      await expect(service.signup(mockGuest)).rejects.toThrowError(ConflictException)
    })
  })
  ...
  describe('signin', ()=>{
   const mockGuestSignin = {
    ...
    it("Should call prisma.findUnique with correct params", async () => {
      ...
      jest.spyOn(prismaService.guest, "findUnique").mockImplementation(mockPrismaSigninGuest)
    ...
    it("Should throw HTTP Exception: invalid credentials if email or password are incorrects", async()=>{
      ...
      jest.spyOn(prismaService.guest, "findUnique").mockImplementation(mockPrismaGuest)

      await expect(service.signin({email:"mei123@outlook.com", password:"mei123456"}))
